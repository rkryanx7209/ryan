...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // SQL CORRIGIDO: Removida a vírgula após 'genero' e após ':genero'
    $sql = "INSERT INTO clientes (nome, email, senha, idade, telefone, genero) 
            VALUES (:nome, :email, :senha, :idade, :telefone, :genero)";
    ...
    try {
        $stmt->execute([
            ':nome'     => $nome,
            ':email'    => $email,
            ':senha'    => $senha, // DICA: É recomendável usar password_hash($senha, PASSWORD_DEFAULT)
            ':idade'    => $idade,
            ':telefone' => $telefone,
            ':genero'   => $genero,
        ]);
        ...
        // Exibe o erro real para você debugar se houver outro problema (ex: nome de coluna errado)
